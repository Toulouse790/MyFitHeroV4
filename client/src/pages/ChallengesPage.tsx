...
async function fetchChallenges({
  ...
  let query = supabase.from<Challenge>("challenges").select("*", { count: "exact" })
...
function useChallenges(params: FetchChallengesParams) {
  const key = ["challenges", params.page, params.pageSize, params.searchTerm, params.filterType?.join(",")];
...
const ChallengesPage: React.FC = () => {
  ...
  return (
    <main className="min-h-screen p-6 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 max-w-6xl mx-auto">
      ...
      <fieldset className="flex flex-wrap justify-center gap-3 mb-6" aria-label="Filtres types de défis">
          ...
          return (
            <label
              ...
              className={clsx(
                ...
                checked ? "bg-blue-600 text-white border-blue-600" : "border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300"
              )}
            >
              <input
                type="checkbox"
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {isLoading && (
          Array.from({ length: PAGE_SIZE }).map((_, idx) => (
            <div key={idx} className="p-6 border rounded-md bg-gray-200 dark:bg-gray-700 animate-pulse h-48" />
          ))
        )}
        {isError && <div className="col-span-full text-center text-red-600">Erreur de chargement des défis.</div>}
        {!isLoading && challenges.length === 0 && <div className="text-center text-gray-600">Aucun défi trouvé.</div>}
        ...
        {challenges.map((challenge) => {
          ...
          return (
            <motion.article
              ...
              <p className="flex-grow text-gray-700 dark:text-gray-300 mb-4 line-clamp-4">{challenge.description}</p>
              <div className="mb-4 text-sm">
                <span><strong>Type :</strong> {challenge.challenge_type}</span>
                <br />
                <span><strong>Objectif :</strong> {challenge.goal_type} - {challenge.goal_target}</span>
                <br />
                <span>
                  Période : {new Date(challenge.start_date).toLocaleDateString()} - {new Date(challenge.end_date).toLocaleDateString()}
                </span>
              </div>
              <button
                ...
                className={clsx(
                  ...
                  joined ? "bg-red-600 hover:bg-red-700 text-white" : "bg-green-600 hover:bg-green-700 text-white"
                )}
                aria-pressed={joined}
                aria-label={joined ? `Se désinscrire du défi ${challenge.title}` : `S'inscrire au défi ${challenge.title}`}
              >
      ...
      {/* Pagination */}
      <nav className="flex justify-center items-center mt-8 space-x-4" aria-label="Pagination des défis">
        <button
