...
export type ValidationRule =
  ...
  | { type: 'custom'; message: string; validator: (value: any, data?: OnboardingData) => boolean | Promise<boolean> }
  | { type: 'conditional'; condition: (data: OnboardingData) => boolean; rule: ValidationRule; message: string }
  | { type: 'async'; message: string; validator: (value: any, data?: OnboardingData) => Promise<boolean> };
...
export interface OnboardingProgress {
  ...
  moduleSpecificSteps: Record<ModuleId, {
    steps: string[];
    completed: string[];
    skipped: string[];
    timeSpent: number;
...
export interface OnboardingData {
  ...
  lifestyle?: 'student' | 'office_worker' | 'physical_job' | 'parent' | 'retired' | 'athlete' | 'other';
  ...
  mainObjective?: 'performance' | 'health_wellness' | 'body_composition' | 'energy_sleep' | 'strength_building' | 'endurance_cardio' | 'recovery_focus' | 'weight_management' | 'muscle_gain' | 'weight_loss' | 'holistic';
  ...
  /* ===== MODULE SPORT ===== */
  sport?: string;
  sportCategory?: 'individual' | 'team' | 'combat' | 'endurance' | 'strength' | 'flexibility' | 'mixed';
  sportPosition?: string;
  sportLevel?: 'recreational' | 'amateur_competitive' | 'club_competitive' | 'semi_professional' | 'professional';
  seasonPeriod?: 'off_season' | 'pre_season' | 'early_season' | 'in_season' | 'championship' | 'recovery';
  ...
  /* ===== MODULE STRENGTH ===== */
  strengthObjective?: 'strength' | 'power' | 'hypertrophy' | 'injury_prevention' | 'endurance' | 'functional';
  strengthExperience?: 'complete_beginner' | 'beginner' | 'intermediate' | 'advanced' | 'expert';
  equipmentLevel?: 'no_equipment' | 'minimal_equipment' | 'home_gym_basic' | 'home_gym_complete' | 'commercial_gym';
  ...
  /* ===== MODULE NUTRITION ===== */
  dietaryPreference?: 'omnivore' | 'vegetarian' | 'vegan' | 'pescatarian' | 'flexitarian' | 'keto' | 'paleo' | 'mediterranean' | 'intermittent_fasting';
  nutritionObjective?: 'muscle_gain' | 'weight_loss' | 'maintenance' | 'performance' | 'recomposition';
  ...
  sleepDifficulties?: ('falling_asleep' | 'staying_asleep' | 'early_waking' | 'restless_sleep' | 'snoring' | 'nightmares')[];
  sleepEnvironment?: {
    darkness: 1 | 2 | 3 | 4 | 5;
    noise: 1 | 2 | 3 | 4 | 5;
  ...
  healthConditions?: ('none' | 'joint_issues' | 'back_problems' | 'cardiovascular' | 'diabetes' | 'hypertension' | 'asthma' | 'arthritis' | 'other')[];
  medications?: {
    name: string;
    dosage?: string;
    frequency: string;
 ...
 */
export type StepResponse<T extends ConversationalStep> = 
  T['inputType'] extends 'multi-select' 
