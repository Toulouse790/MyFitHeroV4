...
export function useSleepAnalysis() {
  ...
  const sleepStats = useMemo((): SleepStats => {
    ...
    // Moyennes
    const avgDuration = recentEntries.reduce((sum, entry) => sum + entry.sleep_duration_minutes, 0) / recentEntries.length;
    const avgQuality = recentEntries.reduce((sum, entry) => sum + entry.sleep_quality, 0) / recentEntries.length;
    ...
    const avgBedtime = `${Math.floor(avgBedtimeMinutes / 60).toString().padStart(2, '0')}:${Math.floor(avgBedtimeMinutes % 60).toString().padStart(2, '0')}`;
    const avgWaketime = `${Math.floor(avgWaketimeMinutes / 60).toString().padStart(2, '0')}:${Math.floor(avgWaketimeMinutes % 60).toString().padStart(2, '0')}`;
    ...
    const waketimeVariance = waketimes.reduce((sum, time) => sum + Math.pow(time - avgWaketimeMinutes, 2), 0) / waketimes.length;
    ...
    const firstHalfAvg = firstHalf.reduce((sum, entry) => sum + entry.sleep_quality, 0) / firstHalf.length;
    const secondHalfAvg = secondHalf.reduce((sum, entry) => sum + entry.sleep_quality, 0) / secondHalf.length;
    ...
    const weeklyAverage = weeklyEntries.reduce((sum, entry) => sum + entry.sleep_duration_minutes, 0) / weeklyEntries.length;
  ...
  const addSleepEntry = useCallback(async (entry: Omit<SleepEntry, 'id' | 'user_id' | 'created_at' | 'sleep_duration_minutes'>) => {
    if (!appStoreUser?.id) {
  ...
  const updateSleepEntry = useCallback(async (entryId: string, updates: Partial<SleepEntry>) => {
    setIsLoading(true);
    try {
  ...
  const updateSleepGoals = useCallback(async (newGoals: Partial<SleepGoals>) => {
