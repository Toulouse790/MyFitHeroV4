...
import type { NutritionData, Meal, FoodItem, NutritionGoals, NutritionAnalysis, Recipe, MealPlan, NutritionTrend, CreateMealDTO, UpdateNutritionGoalsDTO, FoodSearchQuery, NutritionInsight } from '../types/index';
...
export function useNutrition(userId?: string): UseNutritionReturn {
  ...
  const loadAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const analysisData = await NutritionService.getNutritionAnalysis(currentUserId, startDate, endDate);
  ...
  // Ajout d'un repas
  const addMeal = useCallback(async (meal: NutritionMealInput) => {
    if (!currentUserId) return;
  ...
  const updateMeal = useCallback(async (id: string, meal: Partial<NutritionMealInput>) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors de la mise à jour du repas';
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const deleteMeal = useCallback(async (id: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors de la suppression du repas';
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const updateGoals = useCallback(async (newGoals: UpdateNutritionGoalsDTO) => {
    if (!currentUserId) return;
  ...
  const searchFoods = useCallback(async (query: FoodSearchQuery) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors de la recherche d\'aliments';
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const loadRecipes = useCallback(async (filters?: { tags?: string[]; difficulty?: string }) => {
    if (!currentUserId) return;
  ...
  const loadMealPlans = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors du chargement des plans de repas';
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const loadTrends = useCallback(async (period: 'week' | 'month' | 'quarter') => {
    if (!currentUserId) return;
  ...
  const getMacroBalance = useCallback((): { proteins: number; carbohydrates: number; fats: number } => {
