          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center min-w-0 flex-1 py-2 px-1 transition-all duration-300 ${
                active ? `${item.color} scale-110` : 'text-gray-400'
              }`}
            >
              <Icon size={22} className={`transition-all duration-300 ${active ? 'mb-1' : ''}`} />
              ...
              {active && (
                <div
                  className={`w-1 h-1 rounded-full mt-1 ${item.color.replace('text-', 'bg-')} animate-pulse`}
                ></div>
              )}
            </Link>
          );
