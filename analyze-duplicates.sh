...
# Analyser les services dupliqués
echo "## 🔧 SERVICES DUPLIQUÉS" >> $REPORT_FILE
echo "" >> $REPORT_FILE

echo "### Services Supabase" >> $REPORT_FILE
find $CLIENT_DIR -name "*supabase*Service*" -type f | while read file; do
...
find $CLIENT_DIR -name "*api*Service*" -o -name "*Api*Service*" -type f | while read file; do
    lines=$(wc -l < "$file" 2>/dev/null || echo "0")
    echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
done
echo "" >> $REPORT_FILE
...
find $CLIENT_DIR -name "*auth*Store*" -o -name "*Auth*Store*" -type f | while read file; do
    lines=$(wc -l < "$file" 2>/dev/null || echo "0")
    echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
done
echo "" >> $REPORT_FILE
...
find $CLIENT_DIR -name "*app*Store*" -o -name "*App*Store*" -type f | while read file; do
    lines=$(wc -l < "$file" 2>/dev/null || echo "0")
    echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
done
echo "" >> $REPORT_FILE
...
find $CLIENT_DIR -name "*workout*" -name "*.ts" -type f | grep -E "(type|interface)" | while read file; do
    lines=$(wc -l < "$file" 2>/dev/null || echo "0")
    echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
done
echo "" >> $REPORT_FILE
...
find $CLIENT_DIR -name "*onboarding*" -name "*.ts" -type f | while read file; do
    lines=$(wc -l < "$file" 2>/dev/null || echo "0")
    echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
done
echo "" >> $REPORT_FILE
...
grep -r "import.*supabase.*Service" $CLIENT_DIR --include="*.ts" --include="*.tsx" | head -10 >> $REPORT_FILE
echo '```' >> $REPORT_FILE
echo "" >> $REPORT_FILE

echo "### Imports de stores Auth" >> $REPORT_FILE
echo '```bash' >> $REPORT_FILE
grep -r "import.*auth.*Store" $CLIENT_DIR --include="*.ts" --include="*.tsx" | head -10 >> $REPORT_FILE
echo '```' >> $REPORT_FILE
echo "" >> $REPORT_FILE

# Analyser les composants potentiellement dupliqués
...
find $CLIENT_DIR -name "*.tsx" -type f | xargs basename -s .tsx | sort | uniq -d | while read comp; do
    echo "### Composant: $comp" >> $REPORT_FILE
    find $CLIENT_DIR -name "$comp.tsx" -type f | while read file; do
        lines=$(wc -l < "$file" 2>/dev/null || echo "0")
        echo "- \`$file\` ($lines lignes)" >> $REPORT_FILE
    done
    echo "" >> $REPORT_FILE
